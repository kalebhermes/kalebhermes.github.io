...
function getHeaderBar(thisMagicItem) {
	var template = '<div style="background-color:#caa143;color:#FFFFFF;padding:6px 0px 0px 10px;height:24px;">';
		...
		template += '</div>';
		if(thisMagicItem.flavorText) template += '<div>' + '<div style="background-color:#DDDDCC;background-image:url(images/gradient.jpg);font-style:italic;padding: 2px 0px 0px 10px;margin: 2px 0px 0px 0px;font-family:Mentor;">' + thisMagicItem.flavorText + '</div>' + '</div>';
...
function getLevelBonusCost(thisMagicItem) {
	var template = '<div style="display:inline-block;width:375px;margin:5px 0px 0px 0px;font-family:Mentor-Sans;"><table style="border-style:none;border-collapse:collapse;margin:0px 0px 0px 10px;width:165px;display:inline-block;">';
	var templateEnd = '</div>';
	var tableEnd = '</table>';
	var divEnd = '</div>';
	...
	if(numColumns[1] != 0) {
		...
		for(var x = 0; x < numColumns[1]; x++) {
			...
			template += '<tr><td style="border-style:none;padding:0px 0px 0px 0px;width:55px;">Lvl ' + (thisMagicItem.levelBonusCostArray[index].level || '') + '</td><td style="border-style:none;padding:0px 0px 0px 0px;width:25px">+' + (thisMagicItem.levelBonusCostArray[index].bonus || '') + '</td><td style="border-style:none;padding:0px 0px 0px 0px;width:102px;text-align:right;">' + (thisMagicItem.levelBonusCostArray[index].cost || '') + ' gp</td></tr>';
		}
	}
...
function getPowers(thisMagicItem) {
	var template = '<div style="background-color:#DDDDCC;background-image:url(images/gradient.jpg);padding: 2px 0px 0px 10px;">';
...
function getSource(thisMagicItem) {
